...
imgC = len(ImgR)
layout = [  [sg.Text('Number of images to insert:' + str(imgC)), sg.Checkbox("Dev",key='_DCH_')],
            ...
            [sg.Button('Run!'), sg.Button('Quit'), sg.Button('GUILibSettings', key='_GUIL_',disabled=GUID)] ]
layoutS = [  [sg.Text('Please wait attempting to get STI directory.')],
             ...
             [sg.Button('Quit',key='_h2_'), sg.ProgressBar(10,'horizontal',s=(10,10),bar_color=("blue","white"),expand_x=True,key='progressbar')] ]            
...
try:
    ...
    progress_bar.Update(x+0.5) #2.5
    LibraryP = vdf.parse(open(Library))
    progress_bar.Update(x+0.5) #3
    fO = open("./Library.json", "w")
    fO.write(str(LibraryP))
    fO.close()
    progress_bar.Update(x+0.5)#3.5
    fO2 = open("./Library.json", "r")
    filedata = fO2.read()
    fO2.close()
    filedata = filedata.replace('\'', '\"')
    progress_bar.Update(x+0.5)#4
    fO3 = open("./Library.json", "w")
    fO3.write(filedata)
    fO3.close()
    progress_bar.Update(x+1)#5
    FO = open("./Library.json", "r")
    LibraryJ1 = json.loads(FO.read())
    FO.close()
    progress_bar.Update(x+1.5) #6.5
    print("firrst except end")
    ...
    try:
        ...
        SteamINSD = winreg.QueryValueEx(SteamReg, "InstallPath")
        progress_bar.Update(x+0.5) #1
        SteamST = str(SteamINSD)
        SteamST = SteamST.removeprefix("('")
        SteamST = SteamST.removesuffix("', 1)")
        progress_bar.Update(x+1) #2
        SteamINS = SteamST
        #install directory gotten, just need to go into the steamapps folder and get libraryfolders.??? and parse it as json, get the id and in what path it is, then path trought
        #those files for the image folder.
        Library = SteamST + "\\steamapps\\libraryfolders.vdf"
        ...
        exit(2)      
time.sleep(1)
progress_bar.Update(x+6)
time.sleep(1)
progress_bar.Update(x+10)
time.sleep(1)     
window2.close()       
...
while True:
    ...
    if event == sg.WIN_CLOSED or event == 'Quit': # if user closes window or clicks cancel
        break
    window2.Element('_IMAGE_').UpdateAnimation(gif103,  time_between_frames=50)
